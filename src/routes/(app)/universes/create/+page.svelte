<script lang="ts">
	...
	function parseInitialTraits(): TraitDef[] {
		try {
			const raw = form?.values?.traitDefinitions;
...
<section class="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
	...
	<form class="forge-panel space-y-5 p-8" method="POST">
		...
		<div class="flex gap-3">
			<a class="forge-button-ghost flex-1 text-center" href={resolve('/universes')}>Cancel</a>
			<button class="forge-button flex-1" type="submit">Create universe</button>
		</div>
	</form>
</section>
